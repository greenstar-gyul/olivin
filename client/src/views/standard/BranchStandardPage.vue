...
const saveData = async (inputData) => {
    try {
        ...
        const branchData = {
            ...inputData,
            compType:
                isUpdateMode && selectedBranch.value.compType === COMPANY_TYPES.INACTIVE
        ...
        if (response.data.result_code === 'SUCCESS') {
            alert(isUpdateMode ? `지점이 성공적으로 수정되었습니다. (수정자: ${currentUserData.empName})` : `지점이 성공적으로 등록되었습니다. (등록자: ${currentUserData.empName})`);
...
const deleteData = async () => {
    ...
    // ✅ 비활성화 확인 메시지
    const confirmDeactivate = confirm(`지점 "${selectedBranch.value.compName}"을(를) 비활성화하시겠습니까?\n\n비활성화된 지점은 더 이상 사용할 수 없습니다.`);
...
<template>
    <StandardInput ref="standardInputRef" :filters="filters" :items="items" :header="header" :inputs="inputs" @searchData="searchData" @saveData="saveData" @openSearchModal="openSearchModal" @rowSelect="onRowSelect" @rowUnselect="onRowUnselect">
        <!-- ✅ 비활성화 버튼 (삭제 대신) -->
        <template #btn>
            <Button label="삭제" severity="danger" class="min-w-fit whitespace-nowrap" outlined :disabled="!selectedBranch" @click="deleteData" />
        </template>
    </StandardInput>
</template>
