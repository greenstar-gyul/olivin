...
const formatDateForOracle = (dateInput) => {
    ...
    try {
        ...
        console.error('날짜 변환 오류:', error, dateInput);
        return null;
    }
};
...
const loadBranches = async (searchParams = {}) => {
    try {
        loading.value = true;
        console.log('🔍 지점 목록 조회 시작...', searchParams);
        ...
        const branches = companies.filter(
            (item) => item.compType === COMPANY_TYPES.BRANCH // 활성 지점만
        );
...
// ✅ 개선된 저장 처리 (명세서에 맞는 ID 생성)
const saveData = async (inputData) => {
    try {
        console.log('=== 지점 저장 시작 ===');
        ...
        if (isUpdateMode) {
            ...
            branchData.regDateStr = regDate;
            delete branchData.compId; // 백엔드에서 COM20001부터 자동생성

            console.log('🆕 신규 등록 데이터:', branchData);
        ...
        if (response.data.result_code === 'SUCCESS') {
            ...
            if (response.data.data?.compId) {
                alert(successMessage + `\n지점 ID: ${response.data.data.compId}`);
            } else {
                alert(successMessage);
            }
...
const deleteData = async () => {
    ...
    try {
        console.log('=== 지점 비활성화 시작 ===');
        console.log('🔧 비활성화 대상:', selectedBranch.value.compId);
        ...
        if (error.response?.status === 404) {
            ...
            let errorMessage = '비활성화 중 오류가 발생했습니다.';
...
onMounted(async () => {
    ...
    try {
        ...
        console.log('✅ 지점 페이지 마운트 완료');
    } catch (error) {
        console.error('❌ 지점 페이지 마운트 실패:', error);
    }
});
</script>
