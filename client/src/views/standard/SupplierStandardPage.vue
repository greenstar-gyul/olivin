...
const saveData = async () => {
  try {
    ...
    if (isUpdateMode) {
      ...
      supplierData.updateUser = currentUserData.empId; // 수정자 ID
      supplierData.updateDate = now; // 수정일시
      supplierData.regDate = regDate; // 등록일은 기존 값 유지
    ...
    if (response.data.result_code === 'SUCCESS') {
      alert(isUpdateMode ? '공급업체가 성공적으로 수정되었습니다.' : '공급업체가 성공적으로 등록되었습니다.');
      clearForm();
      await loadSuppliers();
    } else {
