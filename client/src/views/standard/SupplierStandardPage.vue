...
const saveData = async () => {
  try {
    ...
    if (formData.value.compId && formData.value.compId.trim() !== '') {
      // 수정 모드
      response = await axios.put(`${API_BASE_URL}/${formData.value.compId}`, supplierData);
    } else {
      // 신규 등록 모드
    ...
    if (response.data.result_code === 'SUCCESS') {
      alert(formData.value.compId ? '공급업체가 성공적으로 수정되었습니다.' : '공급업체가 성공적으로 등록되었습니다.');
      clearForm();
      await loadSuppliers();
    } else {
