      ...
      switch (option) {
        ...
        case 'lastQuarter':
          ...
          const lastQuarterEnd = quarter === 0
            ...
            : new Date(today.getFullYear(), quarter * 3, 0)
  ...
  <div class="surface-ground min-h-screen p-6">
      ...
      <div class="mb-4">
        ...
        <div class="grid grid-cols-4 gap-3 mb-4">
          <Button
            ...
            @click="selectedPeriodType = type.value; handlePeriodTypeChange()"
          />
        </div>
        ...
        <div v-if="selectedPeriodType !== 'custom'" class="grid gap-3 mb-4" :class="`grid-cols-${currentDetailOptions.length}`">
          <Button
            ...
            size="large"
            class="h-12 rounded-lg shadow-sm"
            @click="selectedDetailOption = option.value; handleDetailOptionChange(); searchSalesPerformance(); loadDetailData()"
          />
        </div>
        ...
        <div v-if="selectedPeriodType === 'custom'" class="flex items-center gap-4">
          ...
          <Calendar 
            ...
            @update:modelValue="handleCustomDateChange(); searchSalesPerformance(); loadDetailData()"
            selectionMode="range" 
            dateFormat="yy-mm-dd"
            :showIcon="true"
            :showButtonBar="true"
        ...
        <div class="text-center mt-4 p-3 bg-surface-50 dark:bg-surface-800 rounded-lg shadow-sm">
          ...
          <div class="text-surface-900 dark:text-surface-0 font-semibold">
            {{ fromDate ? fromDate.toLocaleDateString('ko-KR') : '' }} ~ {{ toDate ? toDate.toLocaleDateString('ko-KR') : '' }}
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="card rounded-xl shadow-md h-48">
        <div class="flex justify-between items-start h-full">
          <div>
            ...
            <div :class="['text-sm font-medium', getChangeClass(salesSummary.compareData.grossSalesChange)]">
              전기간 대비 {{ formatPercent(salesSummary.compareData.grossSalesChange) }}
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="flex justify-between items-start h-full">
          <div>
            ...
            <div class="text-muted-color text-sm">
              총매출의 {{ salesSummary.grossSales > 0 ? ((salesSummary.returns / salesSummary.grossSales) * 100).toFixed(1) : '0.0' }}%
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card rounded-xl shadow-md h-48">
        <div class="flex justify-between items-start h-full">
          <div>
            ...
            <div :class="['text-sm font-medium', getChangeClass(salesSummary.compareData.netSalesChange)]">
              전기간 대비 {{ formatPercent(salesSummary.compareData.netSalesChange) }}
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="card rounded-xl shadow-sm">
      ...
      <div v-else class="overflow-x-auto">
        ...
        <DataTable 
          :value="detailData" 
          :paginator="detailData.length > 10" 
          :rows="10"
