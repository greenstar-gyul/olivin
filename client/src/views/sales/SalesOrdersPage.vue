...
const filters = ref({
    ...
    filters: [
        ...
        { type: 'dateRange', label: '주문일자', value: '', name: 'soDate', fromPlaceholder: '예: 2025-07-01', toPlaceholder: '예: 2025-07-31' },
        {
            type: 'item-search',
            label: '지점명',
            value: '',
...
const searchData = async (searchValue) => {
    ...
    if (params.soDateFrom && typeof params.soDateFrom !== 'string') {
        ...
        params.soDateFrom = date.getFullYear() + '-' + String(date.getMonth() + 1).padStart(2, '0') + '-' + String(date.getDate()).padStart(2, '0');
    }
    if (params.soDateTo && typeof params.soDateTo !== 'string') {
        const date = new Date(params.soDateTo);
        params.soDateTo = date.getFullYear() + '-' + String(date.getMonth() + 1).padStart(2, '0') + '-' + String(date.getDate()).padStart(2, '0');
    }
...
const confirmDailySettlement = async () => {
    try {
        ...
        // Date 객체를 문자열로 변환
        const dateString = todaySalesData.value.settlementDate.getFullYear() + '-' + String(todaySalesData.value.settlementDate.getMonth() + 1).padStart(2, '0') + '-' + String(todaySalesData.value.settlementDate.getDate()).padStart(2, '0');
...
<template>
    <SearchForm ref="searchFormRef" :filters="filters" @searchData="searchData" @resetSearchOptions="resetSearchData" @open-search-modal="handleOpenModal" :defaultValues="filterOptions" />
    ...
    <BasicTable :data="masterItems" :header="masterHeader" :checked="true" :dataKey="'soId'" @rowSelect="onRowSelect" @rowUnselect="onRowUnselect" :scrollHeight="'200px'">
        <template #btn>
            <div v-if="user?.compId !== 'COM10001'" class="mb-3">
                <Button label="일일 정산" icon="pi pi-calculator" class="p-button-info" @click="openDailySettlement" />
            </div>
        </template>
    </BasicTable>
    ...
    <!-- 주문코드 선택 모달 -->
    <DialogModal v-model:display="orderCodeModalVisible" :items="orderItems" :headers="orderHeaders" title="주문코드 검색" selectionMode="single" @close="closeOrderModal" @confirm="confirmOrderModal" />

    <!-- 지점 선택 모달 -->
    <DialogModal v-model:display="storeModalVisible" :items="storeItems" :headers="storeHeaders" title="지점 검색" selectionMode="single" @close="closeStoreModal" @confirm="confirmStoreModal" />

    <!-- 일일 정산 모달 -->
    <Dialog v-model:visible="dailySettlementVisible" header="일일 정산" :modal="true" :closable="true" :style="{ width: '400px' }" @hide="closeDailySettlement">
        <div class="p-4">
            <!-- 정산 날짜 선택 -->
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2">정산 날짜</label>
                <Calendar v-model="todaySalesData.settlementDate" :showIcon="true" dateFormat="yy-mm-dd" :maxDate="new Date()" @date-select="onSettlementDateChange" placeholder="정산할 날짜를 선택하세요" class="w-full" />
            </div>

            <div class="mb-4" v-if="todaySalesData.settlementDate">
                <h4 class="text-lg font-semibold mb-2">{{ todaySalesData.settlementDate.toLocaleDateString('ko-KR') }} 판매 현황</h4>
                ...
                <div v-if="todaySalesData.isAlreadyClosed" class="mb-3 p-3 border rounded-lg bg-red-50 border-red-200">
                    ...
                    <div class="text-sm text-red-600 mt-1">정산 금액: {{ Number(todaySalesData.closingAmount).toLocaleString() }}원</div>
                </div>
                ...
                <div v-else class="mb-3 p-3 border rounded-lg bg-green-50 border-green-200">
                    ...
                    <div class="text-sm text-green-600 mt-1">정산을 진행할 수 있습니다.</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 mb-6" v-if="todaySalesData.settlementDate">
                ...
                <div class="p-3 border rounded-lg bg-blue-50">
                    ...
                    <div class="text-2xl font-bold text-blue-600">{{ Number(todaySalesData.totalAmount).toLocaleString() }}원</div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-2">
                    <div class="p-2 border rounded bg-green-50">
                        ...
                        <div class="text-sm font-semibold text-green-600">{{ Number(todaySalesData.cashAmount).toLocaleString() }}원</div>
                    </div>

                    <div class="p-2 border rounded bg-yellow-50">
                        <div class="text-xs text-gray-600">계좌이체</div>
                        <div class="text-sm font-semibold text-yellow-600">{{ Number(todaySalesData.transferAmount).toLocaleString() }}원</div>
                    </div>

                    <div class="p-2 border rounded bg-purple-50">
                        <div class="text-xs text-gray-600">카드결제</div>
                        <div class="text-sm font-semibold text-purple-600">{{ Number(todaySalesData.cardAmount).toLocaleString() }}원</div>
                    </div>
                </div>
            </div>
            ...
            <div class="text-center" v-if="todaySalesData.settlementDate">
                ...
                <div class="flex justify-center gap-2">
                    <Button label="취소" icon="pi pi-times" class="p-button-secondary" @click="closeDailySettlement" />
                    <Button v-if="!todaySalesData.isAlreadyClosed" label="정산하기" icon="pi pi-check" class="p-button-success" @click="confirmDailySettlement" />
                </div>
            </div>
        </div>
    </Dialog>
