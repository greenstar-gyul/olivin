<template>
  <div class="surface-ground min-h-screen p-6">
    ...
    <div class="flex justify-between items-start mb-8">
      <div>
        <h1 class="text-surface-900 dark:text-surface-0 text-4xl font-bold mb-2">
          {{ supplierInfo.COMP_NAME || '공급업체' }} SCM 대시보드
        </h1>
        <p class="text-muted-color text-lg mb-4">
          {{ supplierInfo.COMP_TYPE_NAME || '' }} | {{ supplierInfo.ADDRESS || '' }}
          <span v-if="supplierInfo.TOTAL_PURCHASE_COUNT !== undefined" class="ml-4 text-sm">
            (총 발주 건수: {{ supplierInfo.TOTAL_PURCHASE_COUNT }}건)
          </span>
        </p>
        ...
        <div v-if="isSystemAdmin" class="mb-4">
          ...
          <Dropdown 
            ...
            class="w-80"
          />
          <p v-if="availableSuppliers.length === 0 && isSystemAdmin" class="text-red-500 text-sm mt-2">
            등록된 공급업체가 없거나 로딩 중입니다.
          </p>
          <p v-else-if="availableSuppliers.length > 0 && selectedSupplierId" class="text-green-600 text-sm mt-2">
            {{ availableSuppliers.find(s => s.COMP_ID === selectedSupplierId)?.COMP_NAME }} 데이터를 표시하고 있습니다.
          </p>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="card">
        ...
        <div v-if="activeOrders.length === 0" class="text-center text-muted-color p-8 bg-surface-50 dark:bg-surface-800 rounded-lg">
          최근 발주 내역이 없습니다.
        </div>
        <div v-else class="space-y-4">
          <div v-for="order in activeOrders" :key="order.orderId" 
      ...
      <div class="card">
        ...
        <div v-if="topProducts.length === 0" class="text-center text-muted-color p-8 bg-surface-50 dark:bg-surface-800 rounded-lg">
          발주 상품 데이터를 불러오는 중입니다...
        </div>
        <div v-else class="space-y-4">
          <div v-for="(product, index) in topProducts" :key="product.productId" 
               class="flex items-center p-4 bg-surface-50 dark:bg-surface-800 rounded-lg">
            ...
            <div class="flex-1">
              ...
              <div class="text-muted-color text-sm">
                발주량: {{ formatNumber(product.totalQuantity) }}개 | 
                발주액: {{ formatAmount(product.totalAmount) }}
              </div>
            </div>
          </div>
        </div>
...
const fetchCurrentUser = async () => {
  try {
    ...
    // 시스템 관리자 또는 본사 권한 체크
    isSystemAdmin.value = userRole.toUpperCase().includes('SYSTEM_ADMIN') || 
                         userRole.toUpperCase().includes('GENERAL_MANAGER') || 
                         userCompId === 'COM10001' ||
                         userRole.toUpperCase().includes('HEADQUARTERS')
