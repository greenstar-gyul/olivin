...
onBeforeMount(async () => {
  ...
  // 승인, 반려 버튼 컴포넌트 주입
  if (defaultForm.value.orderStatus === '030001') { // 승인대기
    // 사용자 정보
    ...
    if (userCompInfo.data?.compType === '100001') {  // 사용자가 본사인 경우
      defaultForm.value.compType = userCompInfo.data.compType;
      // 버튼
      approvalBtnSlot.value = h(Button, { 
        severity: "info",
        variant: "outlined", 
        onClick: () => approvalDialogHandler()
      }, '승인');
      rejectionBtnSlot.value = h(Button, { 
        severity: "danger", 
        variant: "outlined", 
        onClick: () => rejectionVisible.value = true
      }, '반려');
    }
  } else if (defaultForm.value.orderStatus === '030004') { // 반려
    const res = await axios.get(`/api/orders/${route.params.orderId}/rejection`);
    formSchema.value.push(
      { label: '반려사유', id: 'rejectionReson' },
      { label: '반려일자', id: 'rejectionDate', data: 'date' }
    );
