...
<template>
    <div class="card mt-6">
        ...
        <DataTable v-model:selection="selectedProducts" class="p-datatable-sm" :value="data" :dataKey="'id'" showGridlines scrollable :scrollHeight="maxHeight" :tableStyle="`min-width: 50rem; max-width: ${props.maxWidth};`">
            ...
            <Column v-for="col in props.columns" :key="col.id" :field="col.field" :header="col.header">
                <template #body="slotProps">
                    ...
                    <!-- DatePicker -->
                    <DatePicker v-else-if="col.inputType === 'date'" v-model="slotProps.data[col.field]" :showIcon="true" :showButtonBar="true" dateFormat="yy-mm-dd" :placeholder="col.placeholder || 'Select date...'" />

                    <!-- Select -->
                    <Select v-else-if="col.inputType === 'select'" v-model="slotProps.data[col.field]" :options="col.options" optionLabel="name" :placeholder="col.placeholder || 'Select option...'" />

                    <!-- Number Input -->
                    <InputNumber v-else-if="col.inputType === 'number'" v-model="slotProps.data[col.field]" inputId="integeronly" :fluid="true" :placeholder="col.placeholder || 'Enter number...'" />
