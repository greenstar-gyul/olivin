        ...
        >
            ...
            <Column v-for="item in items" :key="item" :field="item" :header="header.header[item] ?? item" :sortable="header.sortable && header.sortable.includes(item) ? true : false">
                ...
                <template #body="slotProps">
                    <!-- Tag 렌더링이 정의된 경우 -->
                    <div v-if="props.tagRenderer && props.tagRenderer(slotProps.data, item)" :class="header.rightAligned && header.rightAligned.includes(item) ? 'text-right' : ''">
                        <Tag :value="props.tagRenderer(slotProps.data, item).value" :severity="props.tagRenderer(slotProps.data, item).severity" :rounded="true" />
                    </div>

                    <!-- 기본 렌더링 -->
                    <span
                        v-else
                        :class="[header.rightAligned && header.rightAligned.includes(item) ? 'text-right block' : '', props.cellClass ? props.cellClass(slotProps.data, item) : '']"
