...
<template>
    <div class="card flex flex-col gap-4">
        ...
        <!-- 각 행별로 렌더링 -->
        <div v-for="(row, rowIndex) in groupedFilters" :key="rowIndex" class="grid grid-cols-4 gap-2 mb-2">
            <div v-for="(filter, filterIndex) in row" :key="filter.name || filterIndex" class="flex items-center gap-2" :class="filter.type === 'dateRange' || filter.type === 'textarea' ? 'col-span-2' : 'col-span-1'">
                ...
                <InputGroup v-else-if="filter.type === 'item-search'" class="flex-1">
                    <InputText :id="'filter-' + rowIndex + '-' + filterIndex" v-model="searchOptions[filter.name]" :placeholder="filter.placeholder || 'Enter item name...'" :readonly="filter.disabled || false" :disabled="filter.disabled || false" />
                    <Button icon="pi pi-search" class="p-button-outlined" :disabled="filter.disabled || false" @click="openSearchModal(filter.name)" />
                </InputGroup>

                <!-- Default fallback to text input -->
                <InputText v-else :id="'filter-' + rowIndex + '-' + filterIndex" type="text" v-model="searchOptions[filter.name]" :placeholder="filter.placeholder || 'Enter text...'" :disabled="filter.disabled || false" class="flex-1" />
            </div>
