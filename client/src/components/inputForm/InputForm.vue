    ...
    <div class="grid grid-cols-2 gap-4">
      <div v-for="(input, index) in inputs.inputs" :key="input.name || index" class="grid grid-cols-12 gap-2"
        ...
        <div v-if="input.type !== 'textarea'" class="col-span-12 md:col-span-9 flex">
          ...
          v-model="inputDatas[input.name]" :placeholder="input.placeholder || 'Enter text...'" class="flex-1" />
          ...
          v-model="inputDatas[input.name]" :placeholder="input.placeholder || 'Enter number...'" class="flex-1" />

          <!-- Select Input -->
          <Select v-else-if="input.type === 'select'" :id="'input-' + index" v-model="inputDatas[input.name]" :options="input.options"
            optionLabel="name" optionValue="value" :placeholder="input.placeholder || 'Select option...'" class="flex-1" />
