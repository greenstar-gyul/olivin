...
<template>
    ...
    <SearchForm ref="searchFormRef" :filters="props.filters" @searchData="searchData" />

    <div class="grid grid-cols-7 gap-4 mb-4 items-stretch">
        <!-- ✅ BasicTable 사용 (StandardInput과 동일) -->
        <BasicTable :data="props.items" :header="props.header" :checked="true" :checkType="props.checkType" :scrollHeight="props.scrollHeight" @rowSelect="onRowSelect" @rowUnselect="onRowUnselect" class="col-span-4">
            <template #btn>
                <!-- ✅ 일괄 승인/반려 버튼 -->
                <Button label="일괄승인" severity="success" class="min-w-fit whitespace-nowrap" outlined :disabled="!selectedItems" @click="() => handleApprove({ selectedItem: selectedItems, source: 'bulk' })" />
                <Button label="일괄반려" severity="danger" class="min-w-fit whitespace-nowrap" outlined :disabled="!selectedItems" @click="() => handleReject({ selectedItem: selectedItems, source: 'bulk' })" />
            </template>
        </BasicTable>

        <!-- ✅ ApproveInputForm 사용 (InputForm과 동일한 위치) -->
        <ApproveInputForm ref="approveInputFormRef" :inputs="props.inputs" @saveData="saveData" @approve="handleApprove" @reject="handleReject" class="col-span-3" />
    </div>
</template>
